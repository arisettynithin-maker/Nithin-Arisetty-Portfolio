        ...
        <div className="grid items-stretch gap-6 lg:grid-cols-2">
          {projects.map((project, index) => (
            <Reveal key={project.id} delay={0.07 * index} className="h-full">
              <motion.div className="h-full" whileHover={{ y: -4 }} transition={{ duration: 0.24 }}>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <button
                          ...
                          className="rounded-full border border-line bg-white px-3 py-1.5 text-sm font-semibold text-ink transition hover:border-accent/25 hover:bg-accent-soft/35"
                        >
                          Details
                        </button>
                      </div>
                      <div className="space-y-3">
                        <h3 className="min-h-[4.5rem] font-serif text-3xl leading-tight text-ink">{project.title}</h3>
                        <p className="min-h-[5.25rem] max-w-xl text-base leading-7 text-muted">{project.summary}</p>
                      </div>
                    </div>
